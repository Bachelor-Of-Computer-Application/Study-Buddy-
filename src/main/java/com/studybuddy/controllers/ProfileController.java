...
public class ProfileController {
    ...
    public void initialize() {
        ...
        if (currentUser == null) {
            showAlert(Alert.AlertType.ERROR, "Database Error", "No logged-in user. Profile data cannot be loaded.");
            return;
        }
    ...
    private void loadActivityLog() {
        activityListView.setItems(FXCollections.emptyObservableList());
    }

    private void loadDatabaseStats() {
        try {
            int userId = currentUser.getId();
            statNotesCreated.setText(String.valueOf(noteService.countNotesByUser(userId)));
            statResourcesUploaded.setText(String.valueOf(resourceService.countResourcesByUser(userId)));
            statQuestionsAsked.setText(String.valueOf(questionService.countQuestionsByUser(userId)));
            statAnswersPosted.setText(String.valueOf(questionService.countAnswersByUser(userId)));
            statStudyHours.setText("0h");
            ...
            statStudyHours.setText("0h");
            statAchievements.setText("0");
            showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to load profile stats: " + e.getMessage());
        }
    }
    ...
    public void handleSavePersonal() {
        ...
        if (name.isEmpty() || uname.isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Validation Error", "Full Name and Username cannot be empty.");
            return;
        }
        ...
        if (selectedAvatarFile != null) {
            currentUser.setProfileImagePath(selectedAvatarFile.getAbsolutePath());
        }

        boolean success = userService.updatePersonalInformation(currentUser);

        if (success) {
            showAlert(Alert.AlertType.INFORMATION, "Success", "Saved to database successfully");
            loadProfileData();
        } else {
            showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to update personal information in database.");
        }
    }
    ...
    public void handleSaveStudyPreferences() {
        ...
        if (userService.updateStudyPreferences(currentUser)) {
            showAlert(Alert.AlertType.INFORMATION, "Success", "Saved to database successfully");
            loadProfileData();
        } else {
            showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to update study preferences in database.");
        }
    }
    ...
    public void handleSaveAccount() {
        ...
        if (userService.updateEmail(currentUser.getId(), email)) {
            showAlert(Alert.AlertType.INFORMATION, "Success", "Saved to database successfully");
            loadProfileData();
        } else {
            showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to update email in database.");
        }
    }
    ...
    public void handleSaveNotifications() {
        ...
        if (userService.updateEmailSettings(currentUser)) {
            showAlert(Alert.AlertType.INFORMATION, "Success", "Saved to database successfully");
            loadProfileData();
        } else {
            showAlert(Alert.AlertType.ERROR, "Database Error", "Failed to update notification preferences in database.");
        }
    }
